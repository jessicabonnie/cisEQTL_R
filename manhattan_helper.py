...
def determine_table_name(cell_type, exon, cis):
    if exon:
        exonflag = 'exon'
...
def filter_table(table_loc, p_filter, title=True):
    print table_loc
    snp_dict = dict()
    counter =0
    line1 = True
...
def multi_filter_helper(table_loc, p_filter, chromosome):
    ...
    print chr_table_loc
    filter_table(chr_table_loc, p_filter, title=True)
...
def multi_filter(table_loc, p_filter):
    ...
    for i in range(1,24):
        ...
        filter_table(table_loc,p_filter, title)
    #fileconn.close()
...
def main(argv):
    ...
    #exon = True
    #cis = True
    p_filter = determine_filter(cis)
    for cell_type in ['B','CD4','CD8','MONO','NK']:
        print("About to filter cell_type: "+cell_type)
        table_name = determine_table_name(cell_type, exon, cis)
