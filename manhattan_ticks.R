...
achilleas_all <- function(perm=FALSE, exon=FALSE, flag='', file_type='jpeg', ymin=-log(1e-4)){
	for (cell in c("B","CD4","CD8","MONO","NK")){
		achilleas_cis(cell_type=cell, perm=perm,exon=exon,flag=flag,ymin=ymin)
	}

}

# plot Achilleas' cis-EQTL data, taking a perm flag if it is a perm file
achilleas_cis <-function(cell_type = NULL, perm = FALSE, exon = FALSE, flag = NULL, file_type = 'jpeg',colors=c("navy","darkorange"), genomewideline=F,ymin=0,...){
data_folder = "/home/jkb4y/ubs/work/data/Achilleas/cis-eQTLs_Oct2012"
permflag = ''
transflag = 'transcript'
transtitle = 'Transcript'
if (perm==TRUE){
permflag = '_perm'}
data_name = paste0(cell_type,'_cis_transcript_eqtls',permflag,'_all_new_noNA_JB.txt')
if (exon==TRUE){
	transflag = 'exon'
...
#}
if (!(flag=='')){
permflag = paste0(permflag,"_",flag)}
out_folder =file.path("/home/jkb4y/ubs/work/results/Achilleas/cis-eQTLs_Oct2012",transflag,paste0(cell_type,permflag),"Manhattans")
if (! file.exists(out_folder)){
		dir.create(out_folder)
		}
pdf_out_name = paste0(cell_type, "_cis_eqtls_",transflag,permflag,".pdf")
pdf_out_loc = file.path(out_folder, pdf_out_name)
png_out_loc = file.path(out_folder, paste0(cell_type, "_cis_eqtls_",transflag,permflag,".png"))
ps_out_loc = file.path(out_folder, paste0(cell_type, "_cis_eqtls_",transflag,permflag,".ps"))
jpg_out_loc = file.path(out_folder, paste0(cell_type, "_cis_eqtls_",transflag,permflag,".jpeg"))
if (file_type == 'jpeg'){out_loc = jpg_out_loc}
else if (file_type == 'ps'){out_loc = ps_out_loc}
#out_loc = file.path(out_folder, out_name)
print(out_loc)

title = paste(cell_type,"Cells: Local ",transtitle," eQTLs")
if (perm){title = paste(title, "(perm)")}
...
achilleas_trans <-function(cell_type = NULL, flag = NULL, file_type = 'jpeg',colors=c("navy","darkorange"), genomewideline=F,ymin=0,...){
data_folder = "/home/jkb4y/ubs/work/data/Achilleas/transcript_eqtl_results_Oct12"

data_name = paste0(cell_type,'_all_data_JB.txt')
