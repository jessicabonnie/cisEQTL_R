...
achilleas_all <- function(flag='', file_type='jpeg', ymin=3, onepage=FALSE, filetype='jpeg',ymax=15,genomewideline=T,...){
 	datafolder = "/home/jkb4y/ubs/work/data/Achilleas/eQTLs_Jan2013"
	draw = TRUE
	if (onepage){
		draw=FALSE
		#base_out_loc =file.path("/home/jkb4y/ubs/work/results/Achilleas","cis_trans_Manhattans")
		base_out_loc =file.path("/home/jkb4y/cphgdesk_share/Achilleas/eQTLs_Jan2013/","Manhattans","all_Manhattans")
		if (filetype== 'jpeg'){
	...
	for (exon in c(FALSE, TRUE)){
		...
		for (cis in c(TRUE,FALSE)){
			if (! exon){ ymin = 2}
				...
				print(ymin)
				achilleas_eqtl_draw(cell_type=cell_type,exon=exon, cis=cis, flag=flag, ymax=ymax, draw=draw, ymin=ymin, datafolder=datafolder, genomewideline=genomewideline,...)
	}
	}
	}
	if (onepage){
...
achilleas_eqtl_draw <-function(cell_type = NULL, exon = FALSE, cis = TRUE, flag = NULL, file_type = 'jpeg',colors=c("navy","darkorange"), draw=TRUE, ymin=0, datafolder= "/home/jkb4y/ubs/work/data/Achilleas/eQTLs_Jan2013", genomewideline=T,...){
...
data_name = paste0(exonflag,cislabel,cell_type, filter_label,'.txt')
...
out_folder = file.path("/home/jkb4y/cphgdesk_share/Achilleas/eQTLs_Jan2013","Manhattans")
