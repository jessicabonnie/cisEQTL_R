...
achilleas_all <- function(flag='', file_type='jpeg', ymin=3, onepage=FALSE, filetype='jpeg', ymax="max", genomewideline=T, pca=F, exon=NULL, cis=NULL, chip='I', pca.table.loc=NA, ...){
  source('/h4/t1/users/jkb4y/programs/cisEQTL_R/compare_cells.R')
  folder='eQTLs_June2013'
  datafolder="/home/jkb4y/ubs/work/data/Achilleas/"
  basefolder="/home/jkb4y/cphgdesk_share/Achilleas/"
  basefolder = file.path(basefolder,folder)
  tablefolder = file.path(datafolder, folder)
  pca.table=NULL
  if (!(is.na(pca.table.loc))){
    pca.table<-read.table(file=pca.table.loc,T)
  }
  outfolder <- file.path(basefolder,'mixedpca','Manhattans')
  #outfolder <- locate_subfolder(cis=cis,exon=exon,basefolder=basefolder,pca.table=pca.table,pca=pca)
	#pcaflag = ''
	#if (pca){
...
#transp = -log10(0.05/54821)
	for (exon in exonlist){
		...
		draw=FALSE
		#base_out_loc =file.path("/home/jkb4y/ubs/work/results/Achilleas","cis_trans_Manhattans")
		base_out_loc =file.path(outfolder,"all_Manhattans")
		if (filetype== 'jpeg'){
		...
		#for (cis in cislist){
			...
			#else {ymin = 3}
      if (genomewideline!=F){
				  genomewideline=determine_sigp(chip=chip,cis=cis,exon=exon)
					}
				print(ymin)
				achilleas_eqtl_draw(cell_type=cell_type,exon=exon, cis=cis, flag=flag, ymax=ymax, draw=draw, ymin=ymin, tablefolder=tablefolder, genomewideline=genomewideline, outfolder=basefolder,pca.table=pca.table, ...)
	}
	}
...
achilleas_eqtl_draw <-function(cell_type = NULL, exon = FALSE, cis = TRUE, flag = NULL, file_type = 'jpeg',colors=c("navy","darkorange"), draw=TRUE, ymin=0, tablefolder= NA, genomewideline=T,outfolder=NA,pca.table=NULL,...){
...
data_loc=locate_table(cell=cell_type, cis=cis,exon=exon,tablefolder=tablefolder,pca=NA,pca.table=pca.table,manhattan=TRUE)
print(data_loc)

#out_folder =file.path("/home/jkb4y/cphgdesk_share/Achilleas/eQTLs_Jan2013",exonflag,cisflag,cell_type,"Manhattans")
out_folder = file.path(locate_subfolder(cis=cis,exon=exon,basefolder=outfolder,pca.table=pca.table,pca=pca),"Manhattans")
print(out_folder)
