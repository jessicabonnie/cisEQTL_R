...
data_folder = "/home/jkb4y/ubs/work/data/Achilleas/cis-eQTL_Sept2012"
permflag = ''
if (perm==TRUE){
permflag = '_perm'}
data_name = paste0(cell_type,'_cis_transcript_eqtls',permflag,'_JB.txt')
...
out_folder =file.path("/home/jkb4y/ubs/work/results/Achilleas/cis-eQTL_Sept2012",cell_type,"Manhattans")
out_name = paste0(cell_type, "_cis_eqtls",permflag,".ps")
out_loc = file.path(out_folder, out_name)
print(out_loc)

title = paste(cell_type,"Cells: Local eQTLs")
...
manhattan(results, colors=colors, main=title, suggestiveline=F, genomewideline=genomewideline, pch=16)
dev.off()

}

# manhattan plot using base graphics
manhattan <- function(dataframe, colors=c("navy","orange"), ymax="max", limitchromosomes=1:23,
 suggestiveline=F, genomewideline=F, annotate=NULL, ...) {
